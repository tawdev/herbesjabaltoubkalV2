    ...
    <div className="group flex flex-col gap-4">
      <div className="relative aspect-square overflow-hidden rounded-2xl bg-white p-4 shadow-sm border border-secondary/50 transition-all hover:shadow-xl hover:-translate-y-1">
        ...
        <button 
          ...
          className="absolute bottom-4 right-4 h-12 w-12 rounded-full bg-primary text-primary-foreground opacity-0 transition-all hover:scale-110 group-hover:opacity-100 flex items-center justify-center shadow-xl translate-y-4 group-hover:translate-y-0"
        >
          ...
          <div className="flex flex-col">
            {product.promo_price ? (
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-foreground/60 line-through">{product.price} MAD</span>
              </div>
            ) : (
              <span className="font-bold text-xl text-primary">{product.price} MAD</span>
            )}
            <span className="text-[10px] text-foreground/60 font-bold uppercase tracking-tight">per {product.weight}</span>
          </div>
